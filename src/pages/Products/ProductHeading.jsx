import { API_BASE_URL, API_IMAGE_BASE_URL, BASE_URL } from "@/config/configuration";
...
export default function ProductHeading({
  ...
  const soldCount = getSoldCount(product._id);


  const handleBuyNow = () => {
    const phoneNumber = "971564154420";
    // const phoneNumber = "917034327244";
    ...
    const message = [
      ...
      "",
      "*Product Url:* " + (BASE_URL + "product-detail/" + product?._id),
    ].join("\n");
  ...
  return (
    <div className={`tf-product-heading border-0 ${inStock ? "" : "pb-0 border-0"}`}>
      <span className="brand-product" style={{ textTransform: 'capitalize' }}>{product?.category?.name}</span>
      <h5 className="product-name fw-medium" style={{ textTransform: 'capitalize' }}>{product?.title}</h5>
      ...
      {!inStock ? (
        ...
        <>
          ...
          {soldCount > 0 && (
            <div className="product-stock pb-0">
              ...
              <span className="text-dark"> {soldCount} sold in last 24 hours</span>
            </div>
          )}


          <div className="tf-product-total-quantity mt-3">
            <div
