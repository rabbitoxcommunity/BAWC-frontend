import React from "react";

export default function TextSlider() {
  return (
    <div className="flat-spacing-31">
      <div className="infiniteslide_wrap" style={{ overflow: "hidden" }}>
        <div className="infiniteslide tf-brand style-2 infiniteslider">
          <div
            className="brand-item"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <p className="display-2xl font-10 fw-normal text-green-4">
              Free Shipping
            </p>
          </div>
          <div
            className="brand-item"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <svg
              width={42}
              height={42}
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_6947_2629)">
                <path
                  d="M42 18.9H26.04L37.38 7.56L34.44 4.62L23.1 15.96V0H18.9V15.96L7.56 4.62L4.62 7.56L15.96 18.9H0V23.1H15.96L4.62 34.44L7.56 37.38L18.9 26.04V42H23.1V26.04L34.44 37.38L37.38 34.44L26.04 23.1H42V18.9Z"
                  fill="#F3CA54"
                />
              </g>
              <defs>
                <clipPath id="clip0_6947_2629">
                  <rect width={42} height={42} fill="white" />
                </clipPath>
              </defs>
            </svg>
          </div>
          <div
            className="brand-item"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <p className="display-2xl font-10 fw-normal text-clip-1">
              Shop Now, Pay Later
            </p>
          </div>
          <div
            className="brand-item"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <svg
              width={42}
              height={42}
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_6947_2629)">
                <path
                  d="M42 18.9H26.04L37.38 7.56L34.44 4.62L23.1 15.96V0H18.9V15.96L7.56 4.62L4.62 7.56L15.96 18.9H0V23.1H15.96L4.62 34.44L7.56 37.38L18.9 26.04V42H23.1V26.04L34.44 37.38L37.38 34.44L26.04 23.1H42V18.9Z"
                  fill="#F3CA54"
                />
              </g>
              <defs>
                <clipPath id="clip0_6947_2629">
                  <rect width={42} height={42} fill="white" />
                </clipPath>
              </defs>
            </svg>
          </div>
          <div
            className="brand-item"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <p className="display-2xl font-10 fw-normal text-green-4">
              Free Shipping
            </p>
          </div>
          <div
            className="brand-item"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <svg
              width={42}
              height={42}
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_6947_2629)">
                <path
                  d="M42 18.9H26.04L37.38 7.56L34.44 4.62L23.1 15.96V0H18.9V15.96L7.56 4.62L4.62 7.56L15.96 18.9H0V23.1H15.96L4.62 34.44L7.56 37.38L18.9 26.04V42H23.1V26.04L34.44 37.38L37.38 34.44L26.04 23.1H42V18.9Z"
                  fill="#F3CA54"
                />
              </g>
              <defs>
                <clipPath id="clip0_6947_2629">
                  <rect width={42} height={42} fill="white" />
                </clipPath>
              </defs>
            </svg>
          </div>
          <div
            className="brand-item"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <p className="display-2xl font-10 fw-normal text-clip-1">
              Shop Now, Pay Later
            </p>
          </div>
          <div
            className="brand-item"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <svg
              width={42}
              height={42}
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_6947_2629)">
                <path
                  d="M42 18.9H26.04L37.38 7.56L34.44 4.62L23.1 15.96V0H18.9V15.96L7.56 4.62L4.62 7.56L15.96 18.9H0V23.1H15.96L4.62 34.44L7.56 37.38L18.9 26.04V42H23.1V26.04L34.44 37.38L37.38 34.44L26.04 23.1H42V18.9Z"
                  fill="#F3CA54"
                />
              </g>
              <defs>
                <clipPath id="clip0_6947_2629">
                  <rect width={42} height={42} fill="white" />
                </clipPath>
              </defs>
            </svg>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <p className="display-2xl font-10 fw-normal text-green-4">
              Free Shipping
            </p>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <svg
              width={42}
              height={42}
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_6947_2629)">
                <path
                  d="M42 18.9H26.04L37.38 7.56L34.44 4.62L23.1 15.96V0H18.9V15.96L7.56 4.62L4.62 7.56L15.96 18.9H0V23.1H15.96L4.62 34.44L7.56 37.38L18.9 26.04V42H23.1V26.04L34.44 37.38L37.38 34.44L26.04 23.1H42V18.9Z"
                  fill="#F3CA54"
                />
              </g>
              <defs>
                <clipPath id="clip0_6947_2629">
                  <rect width={42} height={42} fill="white" />
                </clipPath>
              </defs>
            </svg>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <p className="display-2xl font-10 fw-normal text-clip-1">
              Shop Now, Pay Later
            </p>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <svg
              width={42}
              height={42}
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_6947_2629)">
                <path
                  d="M42 18.9H26.04L37.38 7.56L34.44 4.62L23.1 15.96V0H18.9V15.96L7.56 4.62L4.62 7.56L15.96 18.9H0V23.1H15.96L4.62 34.44L7.56 37.38L18.9 26.04V42H23.1V26.04L34.44 37.38L37.38 34.44L26.04 23.1H42V18.9Z"
                  fill="#F3CA54"
                />
              </g>
              <defs>
                <clipPath id="clip0_6947_2629">
                  <rect width={42} height={42} fill="white" />
                </clipPath>
              </defs>
            </svg>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <p className="display-2xl font-10 fw-normal text-green-4">
              Free Shipping
            </p>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <svg
              width={42}
              height={42}
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_6947_2629)">
                <path
                  d="M42 18.9H26.04L37.38 7.56L34.44 4.62L23.1 15.96V0H18.9V15.96L7.56 4.62L4.62 7.56L15.96 18.9H0V23.1H15.96L4.62 34.44L7.56 37.38L18.9 26.04V42H23.1V26.04L34.44 37.38L37.38 34.44L26.04 23.1H42V18.9Z"
                  fill="#F3CA54"
                />
              </g>
              <defs>
                <clipPath id="clip0_6947_2629">
                  <rect width={42} height={42} fill="white" />
                </clipPath>
              </defs>
            </svg>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <p className="display-2xl font-10 fw-normal text-clip-1">
              Shop Now, Pay Later
            </p>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <svg
              width={42}
              height={42}
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_6947_2629)">
                <path
                  d="M42 18.9H26.04L37.38 7.56L34.44 4.62L23.1 15.96V0H18.9V15.96L7.56 4.62L4.62 7.56L15.96 18.9H0V23.1H15.96L4.62 34.44L7.56 37.38L18.9 26.04V42H23.1V26.04L34.44 37.38L37.38 34.44L26.04 23.1H42V18.9Z"
                  fill="#F3CA54"
                />
              </g>
              <defs>
                <clipPath id="clip0_6947_2629">
                  <rect width={42} height={42} fill="white" />
                </clipPath>
              </defs>
            </svg>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <p className="display-2xl font-10 fw-normal text-green-4">
              Free Shipping
            </p>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <svg
              width={42}
              height={42}
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_6947_2629)">
                <path
                  d="M42 18.9H26.04L37.38 7.56L34.44 4.62L23.1 15.96V0H18.9V15.96L7.56 4.62L4.62 7.56L15.96 18.9H0V23.1H15.96L4.62 34.44L7.56 37.38L18.9 26.04V42H23.1V26.04L34.44 37.38L37.38 34.44L26.04 23.1H42V18.9Z"
                  fill="#F3CA54"
                />
              </g>
              <defs>
                <clipPath id="clip0_6947_2629">
                  <rect width={42} height={42} fill="white" />
                </clipPath>
              </defs>
            </svg>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <p className="display-2xl font-10 fw-normal text-clip-1">
              Shop Now, Pay Later
            </p>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <svg
              width={42}
              height={42}
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_6947_2629)">
                <path
                  d="M42 18.9H26.04L37.38 7.56L34.44 4.62L23.1 15.96V0H18.9V15.96L7.56 4.62L4.62 7.56L15.96 18.9H0V23.1H15.96L4.62 34.44L7.56 37.38L18.9 26.04V42H23.1V26.04L34.44 37.38L37.38 34.44L26.04 23.1H42V18.9Z"
                  fill="#F3CA54"
                />
              </g>
              <defs>
                <clipPath id="clip0_6947_2629">
                  <rect width={42} height={42} fill="white" />
                </clipPath>
              </defs>
            </svg>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <p className="display-2xl font-10 fw-normal text-green-4">
              Free Shipping
            </p>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <svg
              width={42}
              height={42}
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_6947_2629)">
                <path
                  d="M42 18.9H26.04L37.38 7.56L34.44 4.62L23.1 15.96V0H18.9V15.96L7.56 4.62L4.62 7.56L15.96 18.9H0V23.1H15.96L4.62 34.44L7.56 37.38L18.9 26.04V42H23.1V26.04L34.44 37.38L37.38 34.44L26.04 23.1H42V18.9Z"
                  fill="#F3CA54"
                />
              </g>
              <defs>
                <clipPath id="clip0_6947_2629">
                  <rect width={42} height={42} fill="white" />
                </clipPath>
              </defs>
            </svg>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <p className="display-2xl font-10 fw-normal text-clip-1">
              Shop Now, Pay Later
            </p>
          </div>
          <div
            className="brand-item infiniteslide_clone"
            style={{ flex: "0 0 auto", display: "block" }}
          >
            <svg
              width={42}
              height={42}
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_6947_2629)">
                <path
                  d="M42 18.9H26.04L37.38 7.56L34.44 4.62L23.1 15.96V0H18.9V15.96L7.56 4.62L4.62 7.56L15.96 18.9H0V23.1H15.96L4.62 34.44L7.56 37.38L18.9 26.04V42H23.1V26.04L34.44 37.38L37.38 34.44L26.04 23.1H42V18.9Z"
                  fill="#F3CA54"
                />
              </g>
              <defs>
                <clipPath id="clip0_6947_2629">
                  <rect width={42} height={42} fill="white" />
                </clipPath>
              </defs>
            </svg>
          </div>
        </div>
      </div>
    </div>
  );
}
